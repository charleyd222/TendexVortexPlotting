...
Vector3f f(const Vector3f& r_V, double R, double S, double vX) {
    Vector3f E;
    double x = r_V[0];
    double y = r_V[1];
    double z = r_V[2];
...
double eigen_solve_val(const Vector3f& r_V, double R, double S) {
    double x = r_V[0];
    double y = r_V[1];
    double z = r_V[2];
    double r = sqrt((z*z) + (y*y) + (x*x));
...
vect rka_iter(double R, double sigma, double vX, double seed_x, double seed_y, double seed_z, int num_its, int icity, double ending_tolerance, double delta_0, double safety, double h0) {    
    ...
    for (i = 0; i < num_its; i++) {
        ...
        while (sizing) {
            ...
            Vector3f k1 = h * f(r, R, sigma, vX);

            Vector3f k2 = h * f(r + b21 * k1, R, sigma, vX);

            Vector3f k3 = h * f(r + b31 * k1 + b32 * k2, R, sigma, vX);

            Vector3f k4 = h * f(r + b41 * k1 + b42 * k2 + b43 * k3, R, sigma, vX);

            Vector3f k5 = h * f(r + b51 * k1 + b52 * k2 + b53 * k3 + b54 * k4, R, sigma, vX);

            Vector3f k6 = h * f(r + b61 * k1 + b62 * k2 + b63 * k3 + b64 * k4 + b65 * k5, R, sigma, vX);
        ...
        double val = eigen_solve_val(r, R, sigma);
