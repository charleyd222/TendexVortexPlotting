...
Matrix3f f(const Vector3f& r_V, double R, double S) {
    Matrix3f E;
    double x = r_V[0];
    double y = r_V[1];
    double z = r_V[2];
...
Vector3f curlCalc(Vector3f r_V, double icity, double R, double S, double h) {
    ...
    Vector3f dF_dx = (eigen_solve(f(r_V + x_change, R, S), icity) - eigen_solve(f(r_V - x_change, R, S), icity)) / (2*h);
    Vector3f dF_dy = (eigen_solve(f(r_V + y_change, R, S), icity) - eigen_solve(f(r_V - y_change, R, S), icity)) / (2*h);
    Vector3f dF_dz = (eigen_solve(f(r_V + z_change, R, S), icity) - eigen_solve(f(r_V - z_change, R, S), icity)) / (2*h);
...
vect rka_iter(double R, double sigma, double vX, double seed_x, double seed_y, double seed_z, 
              int num_its, int icity, double ending_tolerance, double delta_0, double safety, double h0, double d) {    
    ...
    for (i = 0; i < num_its; i++) {
        ...
        while (sizing) {
            ...
            Matrix3f E1 = f(r, R, sigma);
            Vector3f k1 = h * eigen_solve(E1, icity);

            Matrix3f E2 = f(r + b21 * k1, R, sigma);
            Vector3f k2 = h * eigen_solve(E2, icity);

            Matrix3f E3 = f(r + b31 * k1 + b32 * k2, R, sigma);
            Vector3f k3 = h * eigen_solve(E3, icity);

            Matrix3f E4 = f(r + b41 * k1 + b42 * k2 + b43 * k3, R, sigma);
            Vector3f k4 = h * eigen_solve(E4, icity);

            Matrix3f E5 = f(r + b51 * k1 + b52 * k2 + b53 * k3 + b54 * k4, R, sigma);
            Vector3f k5 = h * eigen_solve(E5, icity);

            Matrix3f E6 = f(r + b61 * k1 + b62 * k2 + b63 * k3 + b64 * k4 + b65 * k5, R, sigma);
        ...
        double val = eigen_solve_val(f(r, R, sigma), icity);
        Vector3f e_vecs = eigen_solve(f(r, R, sigma), icity);
