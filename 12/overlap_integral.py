...
if False:
    for l in range(2,lMax):
        ...
        M, L, A_re_mat, A_im_mat, B_re_mat, B_im_mat = load_csv(0.001, lMax, name=f'cppScripts/csvs/xyz_power_1_omega_1/maximized_coefs_xyz_lMax_{l}_Power_1_power.csv')

        m = np.nanmax([A_re_mat, np.imag(A_im_mat), B_re_mat, np.imag(B_im_mat)])
        test = np.abs([A_re_mat, np.imag(A_im_mat), B_re_mat, np.imag(B_im_mat)])
        threshold = np.nanmin(test[np.where(test > 0)])

        norm = colors.SymLogNorm(vmin=-m,vmax=m, linthresh=threshold)
        ...
        fig.savefig(f'coeffecient_plots/{l}_coeffecient_plot.png')
        plt.close()

#M, L, A_re_mat, A_im_mat, B_re_mat, B_im_mat = all_m_check(30)
#M, L, A_re_mat, A_im_mat, B_re_mat, B_im_mat = pm2_m_check(2, lMax)
#M, L, A_re_mat, A_im_mat, B_re_mat, B_im_mat = load_csv(0.001, lMax, name=f'cppScripts/csvs/power_1_omega_1_xyz/maximized_coefs_xyz_lMax_{lMax}_Power_1_power.csv')
M, L, A_re_mat, A_im_mat, B_re_mat, B_im_mat = load_csv(0.001, lMax, name=f'cppScripts/csvs/power_1_omega_1/maximized_coefs_lMax_{lMax}_Power_1_power.csv')
...
if False:
    ...
    ax[0].set_title('A coefficient Real part')
    ax[0].pcolormesh(M, L, A_re_mat[1:], shading='nearest', norm=norm, cmap='jet')
    ax[0].set_xlabel('m')
    ax[0].set_ylabel(r'$\ell$')

    ax[1].set_title('B coefficient Imag part')
    m = ax[1].pcolormesh(M, L, B_im_mat[1:], shading='nearest', norm=norm, cmap='jet')
    ax[1].set_xlabel('m')
    ax[1].set_ylabel(r'$\ell$')

fig.suptitle(r'$A^{\ell m}, B^{\ell m}$ Coeffecients. $F_z$ Maximization Result')
plt.tight_layout()
