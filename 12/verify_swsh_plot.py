...
# Precompute all moble reference values at once
print(f"[INFO] Computing moble reference values for {N_ANGLE}² = "
      f"{N_ANGLE**2} points …", flush=True)

angle_grid = np.array([[t, p] for t in thetas for p in phis])  
...
# 2-D array indexed by [ell_idx, m+ELL_MAX] for convenience
# fill with NaN for invalid (|m| > ell) combinations.
...
m_axis   = np.arange(-ELL_MAX, ELL_MAX + 1)
ell_axis = ells
...
# error vs ell (max over all m and angles)
