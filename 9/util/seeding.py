...
def random_section(seeds,xLim, yLim):
    x_list = []
    y_list = []
    z_list = []
    ...
    for i in range(seeds):
        # Quadrant 1
        x_list += [np.random.random() * xDis + xLim[0]]
        y_list += [np.random.random() * yDis + yLim[0]]
        z_list += [0]
...
def circle(center, seeds, radius):
    
    t = np.linspace(0, 2*np.pi, seeds)
    x = center[0] + radius[0] * np.cos(t)
    y = center[1] + radius[1] * np.sin(t)
    z = np.zeros_like(x)
    return x,y,z
        
def seed(seed_type,distance=10, num = 300, center=(0,0), xLim=0, yLim=0):
    rotate = False
    if seed_type == 0: # Plane
        ...
        x0, y0, z0 = rect(num,distance)
        title = "Outline seeding. RK Adaptive"
    elif seed_type == 6:
        x0,y0,z0 = circle(center, num, distance)
        title = "Circular Outline seeding. RK Adaptive"
    elif seed_type == 7:
        x0,y0,z0 = random_section(num, xLim, yLim)
        title = "Random Section. RK Adaptive"
